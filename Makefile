...
base.tar:
	...
	docker import http://os.archlinuxarm.org/os/ArchLinuxARM-aarch64-latest.tar.gz archlinuxarm:base; docker run --net=host --name archarmwsl archlinuxarm:base /bin/bash -c "pacman-key --init; pacman-key --populate; sed -ibak -e 's/#Color/Color/g' -e 's/CheckSpace/#CheckSpace/g' /etc/pacman.conf; sed -ibak -e 's/IgnorePkg/#IgnorePkg/g' /etc/pacman.conf; userdel -r alarm; pacman -Rs --noconfirm linux-aarch64 mkinitcpio; rm /etc/mkinitcpio.d/linux-aarch64.preset.pacsave; pacman --noconfirm -Syyu; pacman --noconfirm --needed -Sy aria2 aspell ccache dbus dconf dos2unix figlet git grep hspell hunspell inetutils iputils iproute2 keychain libvoikko linux-tools lolcat nano ntp nuspell openssh procps socat sudo usbutils vi vim wget xdg-utils; mkdir -p /etc/pacman.d/hooks; echo '%wheel ALL=(ALL) ALL' > /etc/sudoers.d/wheel; yes | LC_ALL=en_US.UTF-8 pacman -Scc"
